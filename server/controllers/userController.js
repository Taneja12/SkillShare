...
const matchUsers = async (req, res) => {
  try {
      ...
      // Fetch the current user's data including skills to teach and skills to learn
      const currentUser = await User.findById(userId).select('username skillsToLearn skillsToTeach email phoneNumber'); // Add other fields if needed
      ...
      }).select('username skillsToTeach skillsToLearn email phoneNumber'); // Select the fields to return
      ...
      const response = {
          ...
          matchedUsers: validMatches, // Return valid matched users
      };
